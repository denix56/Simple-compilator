...
int Expression::calculate(int a, int b, char op, tableEntry t[N], long SML[N], int flags[N], int &varOrder, int &memOrder)
{
	...
	case '^':
		...
		SML[memOrder++] = BRANCHZERO * 1000 + memOrder + 8; //  проход
		...
		SML[memOrder++] = LOAD * 1000 + varOrder;
		//SML[memOrder++] = BRANCH * 1000 + memOrder + 21;//переход к следующей команде
		return varOrder;
		//корень
	case '\\':
		SML[memOrder++] = LOAD * 1000 + a;//
		...
		SML[varOrder--] = 2;
		//SML[memOrder++] = BRANCHZERO * 1000 + memOrder + 1;//цикл
		SML[memOrder++] = LOAD * 1000 + a;
