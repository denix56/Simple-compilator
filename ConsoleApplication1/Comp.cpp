...
void Comp::if_goto()
{
		...
		strcmp(symb, "<=") == 0 || strcmp(symb, ">=") == 0 || strcmp(symb, "!=") == 0)
	{
		...
		if (strcmp(symb, "==") == 0)
		{
			...
			SML[memOrder++] = BRANCHZERO * 100 + lineAdr;
		}
		else
			if (strcmp(symb, ">") == 0)
			{
				SML[memOrder++] = LOAD * 100 + varAdr2;
				SML[memOrder++] = SUBSTRACT * 100 + varAdr;
				SML[memOrder++] = BRANCHNEG * 100 + lineAdr;
			}
			else
				if (strcmp(symb, "<") == 0)
				{
...
void Comp::next()
{
	...
	SML[memOrder++] = LOAD * 100 + go.varAdr;
	SML[memOrder++] = ADD * 100 + go.stepAdr;
	SML[memOrder++] = STORE * 100 + go.varAdr;
	SML[memOrder++] = BRANCH * 100 + go.branchAdr;
